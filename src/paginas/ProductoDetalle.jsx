    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        if (sortedComments.length > 0) {
          const totalRating = sortedComments.reduce(
            (acc, curr) => acc + (curr.rating || 0),
            0
          );
          const promedio = totalRating / sortedComments.length;
    ...
    <div className="container mx-auto px-4 py-8 sm:py-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 items-start mt-12 md:mt-0">
        ...
        <div className="flex flex-col gap-4">
          <h1 className="text-3xl sm:text-4xl font-bold text-[#9c2007]">
            {producto.nombre}
          </h1>

          {/* ⭐ Resumen compacto: calificaciones + estrellas */}
          <RatingSummary
            averageRating={ratingPromedio}
          ...
          <div className="mt-6">
            ...
            <div className="flex items-stretch gap-4">
              <button
                ...
                className={`bg-[#a34d5f] hover:bg-[#912646] text-white px-8 py-4 rounded-full text-xl transition-all duration-300 shadow-md ${
                  cantidadEnCarrito === 0 ? 'w-full' : 'w-auto'
                } font-semibold`}
              >
                Añadir al Carrito
              </button>
          ...
          {loadingComentarios ? (
            ...
            comentarios.map((comentario) => {
              const puedeGestionar =
                ...
                (usuario.rol === 'admin' ||
                  usuario.rol === 'editor' ||
                  (comentario.autorId && usuario.uid === comentario.autorId));
              const isEditing = editingComment.id === comentario.id;

              return (
                <div
                  key={comentario.id}
                  className="bg-rose-50 p-4 sm:p-5 rounded-2xl shadow-sm border border-rose-100 flex gap-4 items-start"
                >
                      ...
                      <div>
                        ...
                        <div className="flex gap-2 mt-2">
                          <button
                            ...
                            className="bg-[#a34d5f] text-white px-3 py-1 rounded-md text-sm"
                          >
                            Guardar
                          </button>
                          <button
